...
def GridSearchTestsSVCweighted(params, scorer, path, features, feature, randomState=42):
    ...
    Results.to_csv(f'{path}_rmv_{feature}_GridSearchResults.csv')
    ...
    plt.savefig(f'{path}_rmv_{feature}after_CrossValidation.pdf')
    plt.savefig(f'{path}_rmv_{feature}after_CrossValidation.png')
    plt.clf()
    ...
    confusion_matrix_row = {'TN': tn, 'FP': fp, 'FN': fn, 'TP': tp, 'index': f'{features}'}

    # Append the row to the CSV file
    with open('featureComparison.csv', 'a', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=['TN', 'FP', 'FN', 'TP', 'index'])
        writer.writerow(confusion_matrix_row)
...
if __name__ == "__main__":
    ...
    features = ['Area', 'Convexity', 'Rectangularity', 'Elongation', 'Eccentricity', 'Solidity']
    for i, remove in enumerate(features):
        features = ['Area', 'Convexity', 'Rectangularity', 'Elongation', 'Eccentricity', 'Solidity']
        features.remove(remove)
        for j, feature in enumerate(features):
            print(f'Currently at: {i}/{len(features)}')
            featuresReduced = features.copy()
            featuresReduced.remove(feature)
            argument = {'params': {'C': [1], 'gamma': [0.1], 'kernel': ['rbf'],
                                   'class_weight': [None, "balanced", {0: 1, 1: 16}], "random_state": [42]},
                        'scorer': make_scorer(fbeta_score, beta=4),
                        'path': f"Test_Results/SVM_Weights/FeatureComparison/{remove}/",
                        'features': featuresReduced, 'feature': feature}

            p = mp.Process(target=GridSearchTestsSVCweighted, kwargs=argument)
            processes.append(p)
            p.start()
